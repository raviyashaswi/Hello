 ...
 *
 * @param {Object} obj - The object to check for tags.
 * @param {string} tag - The tag to search for.
 * @returns {boolean} True if the object has the specified tag, false otherwise.
 */
function hasTag (obj, tag) {
  return obj && obj.tags && ((Array.isArray(obj.tags) && obj.tags.includes(tag)) || obj.tags === tag);
}

exports.hasTag = hasTag;

function isUsableUserProvidedService (service) {
  return (
    service.label === 'user-provided' &&
    (hasTag(service.credentials, 'hana') ||
      hasTag(service.credentials, 'password'))
  );
}

exports.isUsableUserProvidedService = isUsableUserProvidedService;
/**
 * Extracts the content from VCAP_SERVICES object.
 *
 * @param {Object} services - VCAP_SERVICES parsed
 * @returns {Object} Output ready object
 */
function getFilteredServices (services) {
  const filteredServices = {};

  if (Array.isArray(services['hana']) && services['hana'].length > 0) {
    filteredServices.hana = services['hana'];
  }

  if (Array.isArray(services['user-provided'])) {
    const filteredUserProvided = services['user-provided'].filter(
      (s) => hasTag(s, 'hana') || isUsableUserProvidedService(s)
    );
    if (filteredUserProvided.length > 0) {
      filteredServices['user-provided'] = filteredUserProvided;
    }
  }
...
function extractServiceData (service) {
  // For `other (irrelevant keys)`
  if (typeof service === 'string') {
    return service;
  }
  const outputIfNotEmpty = (value) => value ? value : undefined;
  const c = service.credentials;
  let credentials = undefined;
  if (c) {
    credentials = {
      schema: outputIfNotEmpty(c.schema),
      database_id: outputIfNotEmpty(c.database_id),
      user: outputIfNotEmpty(c.user),
      hdi_user: outputIfNotEmpty(c.hdi_user),
      service_key_name: outputIfNotEmpty(c.service_key_name),
      tags: outputIfNotEmpty(c.tags)
    };
  }
  const data = {
    name: outputIfNotEmpty(service.name),
    label: outputIfNotEmpty(service.label),
    plan: outputIfNotEmpty(service.plan),
    credentials: credentials,
    tags: outputIfNotEmpty(service.tags)
  };
  if (service.name !== service.instance_name) {
    data.instance_name = outputIfNotEmpty(service.instance_name);
  }
  return data;
}

exports.extractServiceData = extractServiceData;
...
/**
 * Returns an appropriate client and sets the logger.client.
 * This function attempts to load either the '@sap/hana-client' or 'hdb' module based on user preference and availability.
 * It also sets the client value in the logger so make this is called before checking logger.client.
 *
 * @returns {Object} The loaded client module (either '@sap/hana-client' or 'hdb')
 * @throws {Error} If neither '@sap/hana-client' nor 'hdb' can be loaded
 */
exports.getClient = function () {
  if (log.getHDBValue()) {
    try {
      const client = require(log.hdb);
      log.setClientValue(log.hdb);
      return client;
    } catch (error) {
      throw new Error(`an error occured while getting the 'hdb' client. Error: ${error.message}`);
    }
  } else {
    try {
      const client = require(log.hana);
      log.setClientValue(log.hana);
      return client;
    } catch (error) {
