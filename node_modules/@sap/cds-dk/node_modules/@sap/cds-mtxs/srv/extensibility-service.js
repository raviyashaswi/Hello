...
class DbAdapter {
  ...
  static async put(ext) {
    ...
    await cds.tx({ tenant, user: new cds.User.Privileged() }, async tx => {
      ...
      try {
        await tx.run(INSERT.into('cds_xt_Extensions').entries({ ID, csn, i18n, sources: ext.archive, activated: this.activated(ext.status), tag, appid: ext.appid ?? APPID, timestamp: new Date().toISOString() }))
      } catch (error) {
        DEBUG?.('Trying legacy request without appid', error)
        await tx.run(INSERT.into('cds_xt_Extensions').entries({ ID, csn, i18n, sources: ext.archive, activated: this.activated(ext.status), tag, timestamp: new Date().toISOString() }))
      }
    })
    return ID
  }
...
module.exports = class ExtensibilityService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.after(['UPDATE', 'UPSERT'], 'Extensions', async (results, req) => { // TODO populate sources
      ...
      req.results = { ID: res?.tag, csn: res?.csn, i18n: res?.i18n !== '{}' ? res?.i18n : undefined, timestamp: res?.timestamp, appid: res?.appid } // REVISIT: return csvs?
    })
    ...
    this.on('READ', 'Extensions', async req => {
      ...
      if (ext) return { ID: ext.tag, csn: ext.csn, i18n: ext.i18n !== '{}' ? ext.i18n : undefined, timestamp: ext.timestamp , appid: ext.appid } // REVISIT: csvs?
      return ext
    })
