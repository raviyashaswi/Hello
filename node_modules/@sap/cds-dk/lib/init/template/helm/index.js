...
// cds10: Remove this plugin and hasHelm doc in projectReader
module.exports = class HelmTemplate extends require('../../plugin') {
    ...
    async run() {
        console.warn(term.warn(`The "cds add helm" command is deprecated and will be removed in the next major release. Please use the "cds add kyma" command instead.`))
        ...
        await copy(join(__dirname, '../kyma/files/values.schema.json')).to('chart/values.schema.json')
        await merge(__dirname, '../kyma/files/Chart.yaml.hbs').into('chart/Chart.yaml', { with: project })
        await merge(__dirname, '../kyma/files/values.yaml.hbs').into('chart/values.yaml', { with: project })
