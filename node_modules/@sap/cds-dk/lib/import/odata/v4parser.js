...
function _captureAnnotationInCSN(targetString, annotationObject, csnDefsTargetObject) {
    ...
    if (annotationObject instanceof Object && !(annotationObject instanceof Array) && !(Object.keys(annotationObject).length === 1 && (Object.keys(annotationObject)[0] === '=' || Object.keys(annotationObject)[0] === 'xpr' || Object.keys(annotationObject)[0] === '#'))) {
        Object.keys(annotationObject).forEach(innerTargetString => {
            _captureAnnotationInCSN(targetString + '.' + innerTargetString, annotationObject[innerTargetString], csnDefsTargetObject);
        })
    } else {
...
function _parseAssociationProperty(foreignReference, associationNode, parserContext) {
    ...
    associationProperty.type = (associationNode.$OnDelete === 'Cascade' || associationNode.$ContainsTarget) ? 'cds.Composition' : 'cds.Association';
    ...
    if (associationNode.$ContainsTarget) {
        parserContext.schemaDetails[schemaName] ??= {}
        parserContext.schemaDetails[schemaName].containment = true
    }
    ...
    // Generate on condition in case $Partner exists, as that indicates a backlink with which the
    // condition can be generated.
    // If not given convert managed associations and compositions in unmanaged with empty key to avoid
    // "generation" of keys, that do not exist in the external service.
    if (associationNode.$Partner && associationProperty.type === 'cds.Composition') {
        associationProperty.on = [
            {ref: [foreignReference, associationNode.$Partner]},
            '=',
            {ref: ['$self']}
        ]
    } else if (common.checkForEmptyKeys(associationProperty, "V4")) {
...
function _generateCSNForBoundActionFunction(actionFunctionObject, kind, parserContext) {
    ...
    if (parameters) {
        ...
        for (let index = 0; index < parameters.length; index++) {
            // skip the first parameter for bound actions, which is the $self parameter
            // that is only implicit in CSN
            if (!isBound || index > 0) {
...
function _parseEntityContainer(csnDefs, entityContainerElements, parserContext) {
    ...
    if (Object.keys(possibleAnnotations).length > 0) {
        ...
        possibleAnnotations = _resolveAnnotationTerm(
 ...
 *
 * Scenarios:
 * 1. Unbound Actions can't have collision. Action name must be unique.
 * 2. Unbound Functions can have collision; we will parse only one function with the same name having no paramter and a generic String return type.
 */
function _populateUnboundActionFunction(isAction, fqName, schemaDataList, parserContext) {
    ...
    if (isAction) {
        if (dataList.length === 1) {
            parserContext.unboundActions[fqName] = actionFunctionObject;
        }
        else {
            throw new Error(messages.UNBOUND_ACTION_COLLISION + `'${actFunName}' of Schema: ${schemaName} `);
        }
    }
    else {
        if (dataList.length > 1) {
            actionFunctionObject['$Parameter'] = [];
            parserContext.unboundFunctions[fqName] = actionFunctionObject;
            parserContext.unboundFunctions[fqName]["@cds.validate"] = false;
            if(parserContext.unboundFunctions[fqName].$ReturnType){
                parserContext.unboundFunctions[fqName].$ReturnType.$Type = "Edm.String";
            }
        }
        else {
            parserContext.unboundFunctions[fqName] = actionFunctionObject;
...
function _collectUnboundActionFunction(schemaDataList, parserContext) {
    for (let actionName of parserContext.actionToActionImportMap.keys()) {
        _populateUnboundActionFunction(true, actionName, schemaDataList, parserContext);
    }
    for (let functionName of parserContext.functionToFunctionImportMap.keys()) {
        _populateUnboundActionFunction(false, functionName, schemaDataList, parserContext);
    }
}
 ...
 * @param {List<Object>} elementDataList, bound action/function details
 * @param {Object} parserContext
 *
 * Scenarios for bound actions, functions:
 * 1. Actions with same name and binding parameter type can have collision and we will parse only one action with that name having no paramter and a genric String return type.
 * 2. Functions can have collision and we will parse only one function with that name having no paramter and a genric String return type.
 * Special scenario is when same function name is bound to different entity as well.
 */
function _collectBoundActionFunction(isAction, currNamespace, elementName, elementDataList, parserContext) {

    for (let i = 0; i < elementDataList.length; i++) {
        let dataObj = elementDataList[i];
        let boundEntityName = dataObj.$Parameter[0].$Type;
        let entityTypeNamespace = currNamespace;
        let [schemaName, entityName] = _extractSchemaNameAndElementName(boundEntityName);

        // binding parameter type should be within the available namespaces in the scope of a document.
        if (entityTypeNamespace !== schemaName && entityTypeNamespace !== parserContext.schemaAliasToNamespace[schemaName]) {

            // checking if the namespace is available in other schema of the document
            if (parserContext.schemaToSchemaDataIndex.hasOwnProperty(schemaName)) {
                entityTypeNamespace = schemaName;
            }
            // checking if the alias is present for the schema name and assigning the actual namespace
            else if (parserContext.schemaAliasToNamespace[schemaName]) {
                entityTypeNamespace = parserContext.schemaAliasToNamespace[schemaName];
            } else {
                throw new Error(messages.INVALID_BOUND_PARAM_TYPE + ` Check the Element: ${elementName} of Schema: ${currNamespace} `);
            }
        }

        if (!parserContext.entityTypes.includes(entityTypeNamespace + '.' + entityName)) {
            throw new Error(messages.UNRESOLVED_TYPE + boundEntityName);
        }
        boundEntityName = entityTypeNamespace + '.' + entityName;

        if (isAction) {
            if (!parserContext.entityToBoundActions[boundEntityName]?.[elementName]) {
                if (!parserContext.entityToBoundActions[boundEntityName]) {
                    parserContext.entityToBoundActions[boundEntityName] = {};
                }
                parserContext.entityToBoundActions[boundEntityName][elementName] = dataObj;
            }
            else {
                parserContext.entityToBoundActions[boundEntityName][elementName]["@cds.validate"] = false;
                parserContext.entityToBoundActions[boundEntityName][elementName]["$Parameter"] = [];
                if (!parserContext.entityToBoundActions[boundEntityName][elementName].$ReturnType) {
                    parserContext.entityToBoundActions[boundEntityName][elementName].$ReturnType = {};
                }
                parserContext.entityToBoundActions[boundEntityName][elementName].$ReturnType.$Type = "Edm.String";
            }
        }
        else {
            if (!parserContext.entityToBoundFunctions[boundEntityName]?.[elementName]) {
                if (!parserContext.entityToBoundFunctions[boundEntityName]) {
                    parserContext.entityToBoundFunctions[boundEntityName] = {};
                }
                parserContext.entityToBoundFunctions[boundEntityName][elementName] = dataObj;
            }
            else {
                parserContext.entityToBoundFunctions[boundEntityName][elementName]["@cds.validate"] = false;
                parserContext.entityToBoundFunctions[boundEntityName][elementName]["$Parameter"] = [];
                parserContext.entityToBoundFunctions[boundEntityName][elementName].$ReturnType.$Type = "Edm.String";
            }
            // throw error if the return type is missing
            if (!parserContext.entityToBoundFunctions[boundEntityName][elementName].$ReturnType) {
...
function _collectSchemaElements(edmxAsJson, schemaDataList, parserContext) {
    // This array is used to store the parameter values which will be used later to call _collectBoundActionFunction.
    let boundActionFunctionDetails = [];

    _collectUnboundActionFunction(schemaDataList, parserContext);

    // capture all the references
    if (edmxAsJson.$Reference) {
        Object.entries(edmxAsJson.$Reference).forEach(ref => {
            parserContext.vocabNamespaceToAlias[ref[1].$Include[0].$Namespace] = ref[1].$Include[0].$Alias;
        });
    }

    // collect entityType, enumType, complexType, typeDefinition, bound action/function, annotations
            ...
            if (elementData instanceof Object) {
                ...
                // bound actions/functions
                    ...
                    if (elementData[0].$IsBound) {
                        boundActionFunctionDetails.push([isAction, currNamespace, elementName, elementData]);
                    }
                }
                // annotations
                else if (elementName === '$Annotations') {
                    schemaData.$Annotations = _resolveAnnotationTerm(JSON.stringify(schemaData.$Annotations), parserContext);
                    /**
    ...
    // This will delay the execution of _collectBoundActionFunction, as we need to collect all the entityTypes first.
    boundActionFunctionDetails.forEach(paramValues => {
        _collectBoundActionFunction(...paramValues, parserContext);
    });
}
...
function _preProcess(edmxAsJson, parserContext, importerContext) {
    ...
    Object.keys(edmxAsJson).forEach(schemaName => {
        ...
        if (ignoreObjects.indexOf(schemaName) === -1 && schemaData instanceof Object) {
            ...
            parserContext.schemaToPrimarySchema[schemaName] = parserContext.primarySchema;
            indexOfSchema++;
        }
...
function _generateCSN(edmxAsJson, ignorePersistenceSkip, parserContext, importerContext) {
    ...
    for (let schemaData of schemaDataList) {
        ...
        // Step 5: parse all bound action & function
        _parseBoundActionFunction(schemaData, csn.definitions, 'action', parserContext);
        _parseBoundActionFunction(schemaData, csn.definitions, 'function', parserContext);
        // Step 6: parse all unbound action & function
        csn.definitions = Object.assign(_parseUnboundActionFunction(schemaData, 'action', parserContext), csn.definitions);
        csn.definitions = Object.assign(_parseUnboundActionFunction(schemaData, 'function', parserContext), csn.definitions);
    ...
    if (parserContext.mockServerUc) {
        ...
        if (parserContext.schemaDetails[parserContext.primarySchema]?.containment) {
            csn.definitions[parserContext.primarySchema]['@odata.containment'] = true;
        }
    ...
    // Step 9: 2. Step for generating $ContainsTarget & $Partner links
    // The containing composition is already generated. Now the Keys: [] has to be removed
    // on the backlink association because per OData spec, OData does not expose the foreign key fields
    // used by the backlink and thus it is save to assume they must be generated for proper mocking
    //
    // This happens after Step 8 because that adjusts the names of entity container entities where
    // entity type and entity set differ in naming
    for (const [eName, entity] of Object.entries(csn.definitions)) {
        if (entity.kind !== 'entity') continue;
        const containingCompositions = Object.keys(entity.elements).filter(e =>
            entity.elements[e].type === 'cds.Composition' &&
            entity.elements[e].on
        );
        for (const comp of containingCompositions) {
            const compTarget = csn.definitions[entity.elements[comp].target];
            const backLink = Object.keys(compTarget.elements).find(e =>
                compTarget.elements[e].type === 'cds.Association' &&
                compTarget.elements[e].target === eName
            )
            if (compTarget.elements[backLink]?.keys) {
                delete compTarget.elements[backLink].keys
            }
        }
    }
        ...
        try {
            ...
            v4Conv.execute(inputBuffer, (error, json, missingReferencedDocuments) => {
                if (missingReferencedDocuments?.length > 0) {
                    console.log(missingReferencedDocuments);
                }
...
async function getEdmxv4CSN(edmx, ignorePersistenceSkip, mockServerUc, importerContext) {
    ...
    const parserContext = ParserContext.getInstance(mockServerUc);
