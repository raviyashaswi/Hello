 ...
 * @param {boolean} [ignoreSymlinks=false] - whether to ignore symlinks
 * @returns {string[]} files found in the given directory and its subdirectories
 */
function getFiles(srcDir, filter, files = [], ignoreSymlinks = false) {
    function handleResource(/** @type {string} entry */ entry) {
        if (!filter || filter.call(this, entry)) {
            ...
            } else if (stats.isFile() || (!ignoreSymlinks && stats.isSymbolicLink())) {
                files.push(entry)
            }
        }
    }
